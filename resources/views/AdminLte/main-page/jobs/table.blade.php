...
@section('head')
    ...
    <title>VicTory Jobs - Admin</title>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.updateForm').submit(function(event) {
                ...
                var salary = form.find('#leader').val().trim();
                var experience = form.find('#experience').val().trim();

                var description = form.find('#description').val().trim();
                var contact = form.find('#contact').val().trim();


                if (name === '' || title === '' || salary === '' || experience === '' || description ===
                    '' || contact === '') {
                    event.preventDefault();
                    Swal.fire({
                        icon: "error",
                ...
                if (title === '') {
                    ...
                    $('#title').addClass('border border-2 border-danger')
                } else {
                    $('#title').removeClass('border border-2 border-danger')
                }
                if (salary === '') {
                    console.log('salary empty')
                    $('#salary').addClass('border border-2 border-danger')
                } else {
                    $('#salary').removeClass('border border-2 border-danger')
                }
                if (experience === '') {
                    console.log('experience empty')
                    $('#experience').addClass('border border-2 border-danger')
                } else {
                    $('#experience').removeClass('border border-2 border-danger')
                }
                ...
                if (contact === '') {
                    ...
                    form.find('.contact-input').find('.note-toolbar').addClass(
                        'border border-2 border-danger')
                    form.find('.contact-input').find('.note-editable ').addClass(
                        'border border-2 border-danger')
                } else {
                    form.find('.contact-input').find('.note-toolbar').removeClass(
                        'border border-2 border-danger')
                    form.find('.contact-input').find('.note-editable ').removeClass(
                        'border border-2 border-danger')
                }
            });
        })
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>Jobs Managerment</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ url('/admin/dashboard') }}">Home</a></li>
                            <li class="breadcrumb-item active">Jobs Manager</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if (
                                            ...
                                            <img alt="Avatar" class="table-avatar w-100"
                                                src="{{ asset('/img/jobs/unknown.png') }}">
                                        @else
                                            <img alt="Avatar" class="table-avatar w-100"
                                    ...
                                    <td class="project-actions text-left">
                                        <a class="btn btn-primary btn-sm "
                                            href="{{ url('/admin/jobs/detail?id=' . $job->id) }}">
                                            <i class="fas fa-folder">
                                            </i>
                                            View
                                        </a>
                                        <button type="button" class="btn btn-info btn-sm" data-toggle="modal"
                                            data-target="#update{{ $job->id }}">
                                            <i class="fas fa-pencil-alt">
                                            </i>
                                            Edit
                                        </button>
                                        <!-- Update modal -->
                                        <div class="modal fade" id="update{{ $job->id }}">
                                            <div class="modal-dialog modal-xl">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Update form for job id:
                                                            {{ $job->id }}</h4>
                                                    ...
                                                    <form class="updateForm"
                                                        action="{{ url('/admin/jobs/proccessUpdate') }}" method="POST"
                                                            ...
                                                            <div class="card-body">
                                                                ...
                                                                <div class="form-group mb-5">
                                                                    ...
                                                                    <select id="city" class="duallistbox"
                                                                        name="cityId">
                                                                        <option value="-1" class="text-danger">*No
